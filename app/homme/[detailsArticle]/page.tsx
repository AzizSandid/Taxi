  ...
  return (
    <div className="flex flex-col items-center py-24 bg-white">
      ...
      <div className="product-detail flex w-11/12 justify-between">
        <div className="images flex flex-col w-[43%] bg-red-600">
          <img
            src={mainImage}
            alt="Main product"
            className="w-[78%] h-[90%] max-w-[516px] max-h-[600px] object-cover mb-4 rounded-none"
          />
          <div className="thumbnail-images flex">
            {detailsArticle.images[selectedColor].slice(1,4).map((img, index) => (
              <img
                key={index}
                src={img}
                alt={`Thumbnail ${index + 1}`}
                className="w-[25%] h-[90%] max-w-[170px] max-h-[170px] object-cover mr-2 cursor-pointer"
                onClick={() => setMainImage(img)}
              />
            ))}
          </div>
        </div>
        <div className="product-info w-[55%] bg-green-600">
